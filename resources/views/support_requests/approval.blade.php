...
<main class="flex-grow"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8"><div class="flex justify-between items-start"><div>
        ...
        <p class="text-gray-600 dark:text-gray-400 mt-1">Review and manage support request approvals</p>
    </div></div></div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
        ...
        @foreach($supportRequests as $request)
            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition">
                ...
                <td class="px-6 py-4"><div class="flex justify-center gap-2"><a href="{{ route('support-requests.approval-show', $request) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800" title="View"><i class="material-icons text-lg">visibility</i></a></div></td>
            </tr>
        @endforeach
        </tbody></table></div><div class="px-6 py-4 border-t border-gray-200 dark:border-slate-600">{{ $supportRequests->links() }}</div>
        @else
        <div class="text-center py-12"><i class="material-icons text-6xl text-gray-300 dark:text-gray-600 mb-4">inbox</i><p class="text-gray-600 dark:text-gray-400 text-lg">No requests found for approval.</p></div>
        @endif
    </div>
</div></main>
