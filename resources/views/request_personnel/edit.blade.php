...
<head>
    ...
    <title>Edit Personnel Request - {{ env('APP_NAME', 'IT Department') }}</title>
...
<body class="flex flex-col min-h-screen pt-16 bg-gray-50 dark:bg-slate-900 transition-colors duration-200">
    ...
    <main class="flex-grow">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <!-- Header Section -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Edit Personnel Request</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-1">Update request details</p>
            </div>

            <!-- Edit Form -->
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-8">
                <form action="{{ route('request-personnel.update', $requestPersonnel) }}" method="post" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <!-- Hidden Requestor Field -->
                    <input type="hidden" name="requestor_id" value="{{ old('requestor_id', $requestPersonnel->requestor_id) }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="start_date_time" id="start_date_time" value="{{ old('start_date_time', $requestPersonnel->start_date_time->format('Y-m-d\TH:i')) }}" class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('start_date_time') border-red-500 @enderror">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="end_date_time" id="end_date_time" value="{{ old('end_date_time', $requestPersonnel->end_date_time->format('Y-m-d\TH:i')) }}" class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('end_date_time') border-red-500 @enderror">
                            @error('end_date_time')
                                <p class="text-red-500 text-sm mt-1 flex items-center gap-1">
                                    <i class="material-icons text-sm">error</i>
                                    {{ $message }}
                    ...
                    <div>
                        ...
                        <input type="text" name="point_person" id="point_person" placeholder="Enter point person name" value="{{ old('point_person', $requestPersonnel->point_person) }}" maxlength="100" class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('point_person') border-red-500 @enderror">
                        @error('point_person')
                            <p class="text-red-500 text-sm mt-1 flex items-center gap-1">
                                <i class="material-icons text-sm">error</i>
                                {{ $message }}
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    @foreach($resources as $resource)
                                        <label class="flex items-center gap-3 p-3 rounded-lg hover:bg-white dark:hover:bg-slate-700 cursor-pointer transition">
                                            <input type="checkbox" name="resources[]" value="{{ $resource->id }}" class="h-4 w-4 text-blue-600 rounded focus:ring-blue-500" {{ (is_array(old('resources')) ? in_array($resource->id, old('resources')) : $requestPersonnel->resources->contains($resource->id)) ? 'checked' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="url" name="meeting_link" id="meeting_link" placeholder="https://example.com" value="{{ old('meeting_link', $requestPersonnel->meeting_link) }}" class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('meeting_link') border-red-500 @enderror">
                        @error('meeting_link')
                            <p class="text-red-500 text-sm mt-1 flex items-center gap-1">
                                <i class="material-icons text-sm">error</i>
                                {{ $message }}
