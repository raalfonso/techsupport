<x-layout>
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-md mx-auto bg-white rounded-lg shadow-md p-6">
            <h1 class="text-2xl font-bold mb-6">Create Auth Item Child</h1>

            <form action="{{ route('auth-child.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    ...
                    <select name="parent" id="parent" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                        ...
                        @foreach($authItems as $item)
                            <option value="{{ $item->name }}" {{ old('parent') == $item->name ? 'selected' : '' }}>{{ $item->name }}</option>
                        @endforeach
                    </select>
                    @error('parent')
                ...
                <div class="mb-6">
                    ...
                    <select name="child" id="child" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                        ...
                        @foreach($authItems as $item)
                            <option value="{{ $item->name }}" {{ old('child') == $item->name ? 'selected' : '' }}>{{ $item->name }}</option>
                        @endforeach
                    </select>
                    @error('child')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
