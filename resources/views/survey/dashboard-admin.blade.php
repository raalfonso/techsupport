...
.comments-container::-webkit-scrollbar-thumb:hover {
        ...
        <div class="mb-8">
            <div class="flex flex-wrap gap-3 items-end">
                <div class="flex-1 min-w-[200px]">
                    ...
                    <select id="department_filter" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
    ...
    <!-- Comments Visual Section -->
    <section id="comments-section" class="py-8" style="background-color: #e6edfc">
      <div class="container mx-auto lg:max-w-screen-xl md:max-w-screen-md px-6 transition-all duration-700 opacity-0 translate-y-4" data-scroll>
        <div class="bg-white rounded-2xl shadow-lg p-6">
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center space-x-3">
                    <div class="bg-blue-100 p-3 rounded-xl">
                        <i class="material-icons text-blue-600 text-2xl">comment</i>
                    </div>
                    <div>
                        <h3 class="text-2xl font-bold text-gray-800">Recent Comments & Feedback</h3>
                        <p class="text-sm text-gray-500">Latest client feedback and ratings</p>
                    </div>
                </div>
                <div class="flex items-center space-x-3">
                    <span class="text-sm text-gray-600 font-medium">Total: <span class="text-blue-600 font-bold comments-total-count">{{ $surveys->filter(fn($s) => !empty($s->comments))->count() }}</span></span>
                </div>
            </div>
            
            <div class="space-y-4 max-h-96 overflow-y-auto pr-2 comments-container" id="comments-list" style="scrollbar-width: thin; scrollbar-color: #CBD5E1 #F1F5F9;">
                @php
                    $recentComments = $surveys->filter(fn($s) => !empty($s->comments))->take(6);
                @endphp
                
                @forelse($recentComments as $comment)
                <div class="bg-gradient-to-r from-gray-50 to-blue-50 border border-gray-200 rounded-xl p-4 hover:shadow-md transition-all duration-300 transform hover:-translate-y-1">
                    <div class="flex items-start space-x-4">
                        <div class="flex-shrink-0">
                            <div class="h-12 w-12 bg-gradient-to-br from-blue-500 to-blue-600 rounded-full flex items-center justify-center shadow-lg">
                                <span class="text-white font-bold text-lg">{{ substr($comment->client_name ?? 'C', 0, 1) }}</span>
                            </div>
                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between mb-2">
                                <div>
                                    <p class="text-sm font-semibold text-gray-900">{{ $comment->client_name ?? 'Anonymous' }}</p>
                            ...
                            <p class="text-sm text-gray-700 leading-relaxed">{{ $comment->comments }}</p>
                            <div class="mt-2 flex items-center space-x-3 text-xs text-gray-500">
                                <span class="flex items-center">
                                    <i class="material-icons text-sm mr-1">person</i>
                                    {{ $comment->surveyEmployee->name }}
                                </span>
                                <span class="flex items-center">
                                    <i class="material-icons text-sm mr-1">schedule</i>
                                    Response: 
                                    @if($comment->response_time == 2)
                                        <span class="text-emerald-600 font-semibold ml-1">Fast</span>
                                    @elseif($comment->response_time == 1)
                                        <span class="text-blue-600 font-semibold ml-1">Good</span>
                                    @else
                                        <span class="text-red-600 font-semibold ml-1">Slow</span>
                                    @endif
                                </span>
                            </div>
                        </div>
                ...
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 text-sm">No comments available yet</p>
                </div>
                @endforelse
            </div>
            
            <div id="comments-view-all" class="mt-4 text-center" style="{{ $recentComments->count() < 6 ? 'display:none;' : '' }}">
                <a href="#survey-table" class="inline-flex items-center text-blue-600 hover:text-blue-700 font-medium text-sm transition-colors">
                    View all comments in detailed table below
                    <i class="material-icons text-lg ml-1">arrow_downward</i>
                </a>
            </div>
        </div>
      </div>
    ...
    <section id="about" class="py-16" style="background-color: #e6edfc">
      <div class="container mx-auto lg:max-w-screen-xl md:max-w-screen-md px-4 transition-all duration-700 opacity-0 translate-y-4" data-scroll>
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-4xl font-bold text-gray-800">Survey Results</h1>
                <p class="text-gray-600 mt-2">Comprehensive feedback and ratings overview</p>
            </div>
            <button id="export-result-survey"
                class="export-result-survey inline-flex items-center px-4 py-2.5 bg-emerald-600 text-white text-sm font-semibold rounded-lg shadow-md hover:bg-emerald-700 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 transition-all duration-200 transform hover:scale-105">
                <i class="material-icons text-lg mr-2">download</i>
                Export Results
            </button>
        </div>
        ...
        if (!startDate || !endDate) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('.comments-total-count').html(response.commentsCount);
                    
                    // Show/hide view all link
                    if (response.commentsCount >= 6) {
                        $('#comments-view-all').show();
                    } else {
                        $('#comments-view-all').hide();
                    }
